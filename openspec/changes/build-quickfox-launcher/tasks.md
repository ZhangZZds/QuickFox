...
- [ ] 2.1 先为 `SearchResult`、`Action`、Provider trait 和 Action dispatcher 写单元测试
- [ ] 2.2 实现统一 `SearchResult`、`Action`、Provider trait 和 Provider registry
- [ ] 2.3 先为查询解析、前缀识别和模式判断写单元测试
- [ ] 2.4 实现查询解析器，支持普通查询、正则前缀、网页搜索前缀和命令前缀
- [ ] 2.5 先为 `Ranker` 排序规则写单元测试
- [ ] 2.6 实现 `Ranker` 边界，结合模糊匹配、精确子串、路径深度和历史权重

## 3. 配置、存储与历史

- [ ] 3.1 先为默认 TOML 配置创建和配置加载写单元测试
- [ ] 3.2 实现 TOML 配置结构、默认配置生成和配置文件读写
- [ ] 3.3 先为无效配置场景写单元测试，包括缺少 `{query}` 的网页搜索模板
- [ ] 3.4 实现配置校验和安全回退错误
- [ ] 3.5 先为 SQLite storage layer 的迁移、读写和测试隔离写测试
- [ ] 3.6 实现 SQLite storage layer，用于索引状态和历史持久化
- [ ] 3.7 先为文件/目录历史和最近 15 条命令历史写单元测试
- [ ] 3.8 实现文件/目录使用历史、命令历史持久化、清空、关闭和最大条数设置

## 4. 索引与搜索

- [ ] 4.1 先为默认索引根目录解析写平台分支测试
- [ ] 4.2 实现 PathAdapter 的 Windows/Linux/macOS 默认路径解析
- [ ] 4.3 先为索引扫描、包含目录、排除目录和排除模式写单元测试
- [ ] 4.4 实现文件/目录名称与路径索引扫描，不读取文件内容
- [ ] 4.5 先为手动刷新和部分目录失败继续处理写测试
- [ ] 4.6 实现手动刷新索引和失败目录报告
- [ ] 4.7 先为普通模糊搜索、显式正则搜索和无效正则反馈写测试
- [ ] 4.8 实现模糊搜索、`re:` 默认正则搜索和可配置正则前缀
