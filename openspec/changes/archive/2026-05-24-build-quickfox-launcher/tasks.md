...
- [x] 7.1 先为搜索输入、结果列表渲染和选中状态写 UI 测试
- [x] 7.2 实现 Compact 启动窗口基础布局
- [x] 7.3 先为上下方向键、Enter 和 Esc 行为写 UI 测试
- [x] 7.4 实现键盘导航、主动作执行和 Esc 关闭
- [x] 7.5 先为右键/快捷键动作菜单写 UI 测试
- [x] 7.6 实现结果动作菜单，支持打开、打开所在目录和复制路径
- [x] 7.7 先为命令模式 preview/确认视图写 UI 测试
- [x] 7.8 实现命令模式 preview/确认 UI
- [x] 7.9 实现基础设置页，覆盖索引目录、前缀、命令执行开关和历史设置

## 8. Tauri 集成

- [x] 8.1 先为前端调用 Rust 命令的契约写 mock 测试
- [x] 8.2 暴露搜索、执行动作、刷新索引、读取/保存配置和历史操作的 Tauri commands
- [x] 8.3 集成全局快捷键注册和启动窗口显示/隐藏逻辑
- [x] 8.4 集成启动时默认配置创建和索引加载
- [x] 8.5 集成手动刷新索引入口和失败反馈
- [x] 8.6 验证开发环境下 macOS 可以运行 Tauri dev 并打开启动窗口
