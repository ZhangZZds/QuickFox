# Windows 手工验收清单

## 快捷键与窗口

- 发布版启动后不弹出额外 cmd/console 窗口
- QuickFox 已在后台运行时再次启动，已有 launcher 被显示并聚焦，任务管理器中仍只有
  一个 QuickFox 进程、一个托盘实例和一套索引任务
- 双击 Shift 能唤起启动窗口
- 设置页“外观”中能把全局唤醒键录制为 `Control+Space` 等组合键；保存后新组合键能唤起/隐藏启动器
- 设置页“外观”中尝试录制 `Alt+Space` 时显示系统占用提示，且不会把它保存为新的唤醒键
- 设置页拒绝只按单个修饰键作为唤醒键
- 将全局唤醒键设置为 `Shift+Space` 后，在任意文本框中用 Shift 输入大写字母、松开后继续输入 Space，不应唤起或隐藏 QuickFox
- 将全局唤醒键设置为 `Shift+Space` 后，故意慢按 Shift、停顿超过半秒再按 Space，不应唤起或隐藏 QuickFox
- 后台窗口再次双击 Shift 能置前并聚焦输入框
- 已显示窗口再次双击 Shift 能隐藏窗口
- 启动器失去焦点时自动隐藏到后台
- Esc 能关闭窗口
- 窗口出现时输入框自动聚焦
- 快速启动窗口内不显示设置按钮
- 快速启动窗口呈现为紧凑浮层，不显示设置页标题栏、设置导航或大块透明桌面背景
- 托盘菜单“设置”打开独立设置窗口，保留系统关闭、缩放和调整大小入口
- QuickFox 隐藏、最小化或在后台时，只保留托盘图标，不在任务栏呈现一个最小化窗口
- 托盘菜单“显示/隐藏 QuickFox”能显示隐藏的启动器，并能隐藏已聚焦的启动器
- 托盘菜单“显示/隐藏 QuickFox”后，再次按全局唤醒键的显示/隐藏行为与普通唤起一致
- 托盘菜单“设置”能打开设置页；关闭设置窗口后再次点击“设置”能重新创建并显示设置窗口

## 搜索与结果

- 普通文件/目录搜索能返回结果
- 正则搜索 `re:` 行为正常
- 输入不存在查询时不会返回明显不相关结果
- `.exe` 和开始菜单 `.lnk` 作为应用结果，且排序优先于普通文件和目录
- 方向键切换结果稳定
- Enter 执行主动作
- 右键菜单能打开所在目录、复制路径
- 右键菜单出现在被点击结果附近
- 空输入时不显示多余结果区域留白

## 网页搜索与索引

- `g 1234` 能生成 Google 搜索结果并用默认浏览器打开
- `ddg 1234` 能生成 DuckDuckGo 搜索结果并用默认浏览器打开
- `bd 1234` 能生成百度搜索结果并用默认浏览器打开
- 首次启动或大文件树启动时，启动器和托盘不被索引过程阻塞
- 首次创建 Windows 配置时，默认 `include_dirs` 包含当前可用的盘符根目录，例如 `C:\` 和 `D:\`；没有可用盘符时才回退当前用户 profile
- 默认全盘扫描只自动加入本地固定磁盘，不加入映射网络盘或临时移动盘；不进入 `Windows`、`ProgramData`、`Program Files`、`PerfLogs`、`Recovery`、Windows 升级目录、`System Volume Information`、`$Recycle.Bin`、`AppData` 和虚拟内存文件；`Users`、Desktop、Documents 中的普通数据仍可搜索
- 从 v1.6.1 升级时，未修改的自动热路径默认迁移为当前盘符范围；修改过 include/exclude、模式、ignore、watcher 或正文范围的配置保持原值
- `balanced` 模式下，应用入口和 Desktop/Downloads 等热路径先可搜索，启动器提示“文件搜索已部分可用”，随后后台补全盘符范围
- `fast` 模式下首次启动只扫描应用入口和热路径，不自动补扫 `D:\` 等配置大根目录；设置页应说明结果范围更窄
- `complete` 模式下首次索引覆盖完整配置范围；设置页应说明首次索引更慢但覆盖更完整
- 从 `fast` 切换到 `balanced` 后点击“保存设置”，按钮依次显示未保存、保存中和已保存；立即关闭并重开设置仍为 `balanced`，重启应用后也保持不变
- 在 `balanced` 下加入 `D:\` 后保存命令应快速返回，设置页显示配置已保存和后台应用状态；扫描期间输入、切换设置分区和关闭窗口不应明显卡顿
- C/D 同时配置时，热路径完成后最多并行扫描 2 个盘；先完成的盘立即显示“可搜索”并能
  返回结果，不等待另一个盘。每约 2,048 条会写入 SQLite/WAL，数据库相关文件和
  accepted 数应持续变化，而不是直到整盘完成才一次写入
- 扫描 D 盘时退出并重启，配置指纹未变化的情况下已完成 root 不应重扫，并应立即恢复
  为可搜索预览；未完成 root 从最后一个目录断点继续，最近 active baseline 始终可搜索
- 让 `D:\` 中一个配置目录不可访问、临时离线或读取失败时，其他可用 root 仍完成并显示 `partial`；配置文件和重开设置仍保留 `D:\`
- 上述部分失败不能再显示 `standby capture handoff requires a recovery scan` 并丢弃整次 C/D 结果；恢复访问后该子树应重新校准，失败期间旧条目继续可搜索
- C/D watcher 独立：让 D 盘离线或不可监听后，C 盘新建文件仍应通过增量更新进入结果；
  设置页只把 D 标记为部分可用/恢复中
- 在上一条 `partial` 状态恢复目录后点击“重试/校准索引”，状态应进入 applying 并最终 applied；旧可用搜索结果在切换期间继续可用
- 在大目录仍扫描时连续保存 `fast`、`balanced`、`fast`，旧 revision 应及时停止，最终状态和重开设置只反映最后一次 `fast`，且不继续扫描 `D:\`
- 设置页显示 Windows 默认全盘、系统目录自动跳过、后台补全、离线位置部分失败和重试说明
- 文件索引未完成时，启动器提示“文件索引正在建立”或对应失败信息
- 文件索引未建立、构建中或失败时，启动器提供“刷新索引”和/或“打开设置”恢复动作
- 后台索引完成后，当前普通查询能刷新为最新搜索结果
- 索引已建立后快速连续输入文件名，输入框不应卡顿，较早查询晚返回时不覆盖最新输入结果
- 基础文件索引可用但内容索引仍在准备时，普通 name/path 查询可返回结果；`content:` 查询显示内容索引准备中的反馈
- 设置页“搜索与索引”分组能触发后台刷新索引，并显示索引状态
- 设置页“搜索与索引”分组能显示配置文件完整路径和索引快照存储路径
- 设置页“搜索与索引”分组显示当前索引模式、阶段、当前扫描位置和逐盘的等待中、扫描中、
  可搜索、部分可用状态，以及各盘已扫描、收录、跳过和失败数量
- 增量刷新后新增/删除文件能反映到搜索结果
- 存在旧索引快照时，重启后能先使用旧索引搜索，同时后台更新索引

## 大索引性能验收

- 使用发布构建，不使用开发服务器或 debug build
- 将 name/path 索引范围配置为真实 Windows C/D 多盘大目录，例如 `C:\Users` 和 `D:\`；记录实际磁盘总量、扫描 root 和排除项
- 等待索引进入 ready 或 refreshing-but-available 状态；记录设置页显示的 entry count、当前 stage、scanned、accepted、skipped、failures
- 打开任务管理器记录 QuickFox 进程常驻内存；200 万文件级目标小于 500MB，硬上限小于 800MB，超过时必须记录截图和复现配置
- 同时记录 `%APPDATA%\QuickFox\quickfox.sqlite`、`-wal`/`-shm` 的峰值；写入期间不得
  出现 GB 级 `quickfox.sqlite-journal`，完成后只保留 active baseline
- 制造低空间条件时，baseline 写入必须在剩余空间低于“估算值 + 5 GiB”前失败；单个
  baseline 估算超过 8 GiB 时必须提示缩小索引目录
- 在启动器连续输入 `agents.md`，特别观察输入到 `m` 时是否卡顿；录屏中应能看出输入框持续响应、旧搜索结果不会覆盖新输入
- 查询 `agents.md`、`agents.m`、`agents`、`type:md agents`、`dir:workspace agents`，目标文件应稳定在前 5 个结果内
- 查询一个不存在的长词，例如 `needle-not-present-987654321`，不应出现明显输入冻结或全量扫描造成的长时间空白
- 基础 name/path 可用但内容索引仍在准备时，普通查询仍返回结果；`content:` 查询显示内容索引准备中或合理降级
- 在 Windows 机器上运行 2,000,000 synthetic 阈值命令并保存输出：

```powershell
cargo test --release --manifest-path src-tauri/Cargo.toml `
  two_million_entry_search_stays_within_latency_budget `
  -- --ignored --nocapture
```

记录 `QUICKFOX_LARGE_INDEX_THRESHOLD` 输出、任务管理器内存截图、`agents.md` 输入录屏和结果截图。若无法在当轮执行，应在 issue 或 QA 记录中明确标注为待验收，不得将其等同于已通过。

## 运行期增量索引（发布阻塞清单）

使用 Windows 发布构建和本地 NTFS 测试目录，至少覆盖 `C:\` 与 `D:\`。每项记录 watcher 交付后的可见耗时、pending/dirty、degradation code、QuickFox RSS，并保存状态/任务管理器截图；截图遮盖用户名与个人路径。

- [ ] C: create：新建文件 10 秒内进入 name/path 结果
- [ ] C: write：覆盖文件后 name/path 批次成功，正文符合范围时随后独立更新
- [ ] C: rename：旧路径消失、新路径出现，不产生重复结果，满足 10 秒硬上限
- [ ] C: delete：删除文件和删除子树均由 tombstone 在 10 秒内隐藏，邻近同前缀目录不受影响
- [ ] D: create/write/rename/delete：在独立 NTFS 卷重复普通变化并记录各项耗时
- [ ] 同盘跨目录 rename：在 C:、D: 内分别跨已监听目录移动文件/目录，最终状态以旧 tombstone + 新 targeted scan 收敛
- [ ] 跨盘 C:→D: move：按平台实际事件序列验证旧 C: 路径删除、新 D: 路径进入，不能同时显示重复项
- [ ] 1000-file Git checkout：创建/覆盖/删除混合 checkout 期间窗口、输入和非文件 Provider 保持响应
- [ ] checkout 收敛：pending 归零后抽查最终文件系统；overflow/channel 满可观察，dirty-root 校准恢复一致
- [ ] sleep/wake：唤醒后 watcher 状态可观察，离线变化最终通过校准恢复
- [ ] D: drive disconnect：断开/卸载 D: 时最近 baseline 继续服务，root 保持 dirty/Degraded，不把全盘条目误删
- [ ] D: reconnect：恢复原盘符后自动或手动校准收敛；盘符改变时按配置语义变更重新保存
- [ ] watcher 初始化失败：模拟无权限/不可监听 root，显示 `watcherInitializationFailed` 和恢复动作
- [ ] watcher 运行失败：运行中断开 backend/root，显示结构化失败且最近 baseline 可用
- [ ] channel overflow：受控制造超过 8192 的事件压力，队列有界并显示 `channelOverflow`/dirty root
- [ ] 10 秒硬上限：持续写入阻止 5 秒静默时，首事件后不超过 10 秒仍有批次进入可搜索视图
- [ ] UI/状态截图：保存 Watching、Calibrating、Degraded/full refresh fallback、恢复后状态和任务管理器 RSS
- [ ] 真实内存：记录空闲 baseline、1000-file 风暴、校准、full refresh、2M ready 各阶段 RSS/peak，超过 500MB 目标或 800MB 硬上限时阻塞发布
- [ ] 正文反馈/队列：准备中、非法查询、reader 失败反馈明确；容量 8 的正文队列满不回滚 name/path
- [ ] 开关与配置 fence：关闭监听不删搜索；重新开启先校准；include/exclude candidate 失败保留旧配置/view
- [ ] NTFS junction：索引 root 内放置指向 root 外的 junction，扫描/校准/删除不得越界索引或把外部目标误删
- [ ] junction rename/delete：重命名或删除 junction 本身后只更新 junction 路径语义，外部真实目录及文件保持不变
- [ ] 正文 GC junction/symlink 安全：伪造受管名称但不满足真实目录/marker/锁条件的条目不得被回收

结果写入 `docs/runtime-incremental-indexing-manual-qa-results-2026-07-31.md` 或发布日期对应记录。Windows 真实桌面、NTFS C:/D、junction 和任务管理器数据缺失时不得声明该能力完成。

## 命令执行

- `>` 查询能进入命令预览
- 关闭命令执行时能看到禁用提示
- 开启命令执行后，普通命令需要确认
- 危险命令会阻止或要求更强确认
- Windows Terminal 能被正确拉起

## 权限与提示

- 提权或系统修改命令能看到风险提示
- 被阻止的命令不会静默执行

## 历史与设置

- 设置页可从托盘菜单打开
- 设置页按“索引”“网页搜索”“历史”“命令安全”“外观”分区
- 设置页没有“返回搜索”按钮；保存后仍停留在设置窗口
- 设置页关键字段旁显示问号说明，鼠标悬停和键盘聚焦时出现提示
- 调整设置窗口宽度和高度时，内容改为单列或内部滚动，保存入口仍可访问且文本不重叠
- 网页搜索分区能新增 DuckDuckGo 等搜索引擎，缺少 `{query}` 的 URL 模板会被阻止保存
- 命令开关状态可生效
- 最近输入条数调整后行为符合预期
- 上/下方向键能召回最近 15 条确认执行过的文件、目录、命令和网页搜索输入
- 只输入但未按 Enter 执行时不会记录历史

发现问题后，建议记录：

- Windows 版本
- 终端版本
- QuickFox 构建类型和 commit
- 索引 entry count、扫描 roots、磁盘规模和任务管理器内存
- `QUICKFOX_LARGE_INDEX_THRESHOLD` 或未运行原因
- 复现步骤
- 预期行为与实际行为
